...
export default {
    ...
    run: async (interaction: ChatInputCommandInteraction) => {
        ...
        if (!Object.keys(rules).includes(moduleName)) {
            return interaction.reply({
                content: 'Invalid module name. Please select a valid module.',
                ephemeral: true,
            });
        }
        ...
        if (!serverRecord || !serverRecord.automod_channel) {
            ...
            return interaction.reply({
                content:
                    'You must set a log channel for auto-moderation actions using the `/logchannel set` command before configuring modules.',
                components: [row],
                ephemeral: true,
            });
        }
        ...
        if (subcommand === 'addmodule') {
            ...
            const updatedModules = serverRecord.modules.filter(
                (mod: string) => mod !== moduleName,
            );

            const ruleIdToRemove = serverRecord.automod_ids?.find(
                async (id) => {
            ...
            if (ruleIdToRemove) {
                try {
                    ...
                    console.error(
                        `Error deleting rule ${ruleIdToRemove}:`,
                        err,
                    );
                    return interaction.reply({
                        content: `Failed to remove the rule for "${moduleName}".`,
                        ephemeral: true,
                    });
                }

                const updatedAutomodIds = serverRecord.automod_ids?.filter(
                    (id: string) => id !== ruleIdToRemove,
                );
                ...
                await interaction.reply({
                    content: `No associated rule found for module "${moduleName}".`,
                    ephemeral: true,
                });
            }
        }
